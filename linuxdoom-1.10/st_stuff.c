...
boolean ST_Responder (event_t* ev)
{
	...
	else if (ev->type == ev_keydown)
	{
		...
		// can't cheat in multiplayer
		if (!netgame)
		{
			// not sure why the 2 lines below have been commented out, I thought vanilla doom did not allow cheats in nightmare
			...
			else if (cht_CheckCheat(&cheat_choppers, ev->data1))
			{
				...
				plyr->powers[pw_invulnerability] = true; // this value is supposed to be how long the power lasts, but it 'true' instead, which makes invulnerability last 1 tic (see https://doomwiki.org/wiki/Chainsaw#Notes)
